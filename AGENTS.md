...
| Delivery repos (this repo, `starwaytrasporti-web`, ...) | Client-specific routes, branding, content, composition |
...
| Full rule text | `.ai/rules/dolcevita-project.md` (arrives when `chore/remove-cursor` merges; it was the Cursor rule `.cursor/rules/dolcevita-project.mdc`) | `bravobyte-ai/rules/<name>.md` |
| Playbooks | none locally yet | `bravobyte-ai/playbooks/<name>.md` |
| Glossary | none locally | `bravobyte-ai/glossary.md` |

## This repo

This is a delivery repo (`dolcevitact-web`). It intentionally does not keep
a full local mirror of `.ai/rules`, `.ai/playbooks`, or `.ai/commands` — see
the summary above for the operative rules, and this repo's `.ai/README.md`
for the inheritance convention. Its own `.ai/` is currently just a scaffold
(no repo-specific rules yet); add them there only once a repo-specific
pattern settles, per the extraction guidance in that README.
